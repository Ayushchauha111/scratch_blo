...
Blockly.goog.provide('Blockly.BlockDragger');

Blockly.goog.require('Blockly.BlockAnimations');
Blockly.goog.require('Blockly.Events.BlockMove');
Blockly.goog.require('Blockly.Events.DragBlockOutside');
Blockly.goog.require('Blockly.Events.EndBlockDrag');
Blockly.goog.require('Blockly.InsertionMarkerManager');

Blockly.goog.require('Blockly.goog.math.Coordinate');
Blockly.goog.require('Blockly.goog.asserts');
...
Blockly.BlockDragger = function(block, workspace) {
   ...
   * @type {!Blockly.goog.math.Coordinate}
   * @private
   */
...
Blockly.BlockDragger.initIconData_ = function(block) {
  ...
  for (var i = 0, descendant; descendant = descendants[i]; i++) {
    ...
    for (var j = 0; j < icons.length; j++) {
      var data = {
        // Blockly.goog.math.Coordinate with x and y properties (workspace coordinates).
 ...
 * @param {!Blockly.goog.math.Coordinate} currentDragDeltaXY How far the pointer has
...
Blockly.BlockDragger.prototype.startBlockDrag = function(currentDragDeltaXY) {
  ...
  if (this.draggingBlock_.getParent()) {
    ...
    var newLoc = Blockly.goog.math.Coordinate.sum(this.startXY_, delta);
...
Blockly.BlockDragger.prototype.maybeDeleteBlock_ = function() {
  ...
  if (this.wouldDeleteBlock_) {
    if (trashcan) {
      Blockly.goog.Timer.callOnce(trashcan.close, 100, trashcan);
    }
 ...
 * @param {!Blockly.goog.math.Coordinate} pixelCoord A coordinate with x and y values
 *     in css pixel units.
 * @return {!Blockly.goog.math.Coordinate} The input coordinate divided by the workspace
 *     scale.
 * @private
 */
Blockly.BlockDragger.prototype.pixelsToWorkspaceUnits_ = function(pixelCoord) {
  var result = new Blockly.goog.math.Coordinate(pixelCoord.x / this.workspace_.scale,
 ...
 * @param {!Blockly.goog.math.Coordinate} dxy How far to move the icons from their
...
Blockly.BlockDragger.prototype.dragIcons_ = function(dxy) {
  ...
  for (var i = 0; i < this.dragIconData_.length; i++) {
    ...
    data.icon.setIconLocation(Blockly.goog.math.Coordinate.sum(data.location, dxy));
  }
};
